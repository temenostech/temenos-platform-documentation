...
After extracting it in your local machine to any location, the bellow folders and files should be available:
...
> [!Note]
> The development package is pre-configured to be extracted in **_C:\TemenosDev_** path. 
> If the package is extracted in another path (Your_DEV_Path), make sure you make the bellow **changes** to the development package:
...
> [!Note] 
> <span style="color:red">If above steps are not fixing the issue, then:</span>
>
> 
> - Delete the **tafj.link** file from the **dropins** folder.
> - Copy the plugins from your **TAFJ> eclipse > plugins** and paste them into the **DesignStudio > dropins** folder of your DS.
> - Example:
> ![jars](./images/t24-dev-copy-jars.png)

<br>

In **tafj.properties** from Your_DEV_Path\TAFJ\conf, update the lines based on Your_DEV_Path.
 -If a project already exists in your DesignStudio workspace, in **Launch T24 Packager (TAFJ).launch** file from Your_DEV_Path\DesignStudio\workspace\YOUR-project-packager, lines related to *home* must be updated accordingly to Your_Path. 
- Example:
![set home](./images/t24-dev-set-home.png)

<br>


To be able to interrogate the sandbox database, a series of updates must be applied to **tafj.properties** file from Your_DEV_Path\TAFJ\conf:  
| **Property**  		| Value	|
|-					|-		|
...
![tafj properties](./images/t24-tafj-properties.png) 

<br>

> [!Note]
> In order to get the **private database IP** of your Sandbox, you need to contact Temenos Cloud support team at [**CloudPlatformSupport@temenos.com**](CloudPlatformSupport@temenos.com).
...
> [!Note]
> For **project-name** parameter, make sure you use small letters (ft-sample as per above example) and for **component-name** parameter, make sure you start the name with capital letters (FT-Sample as per above example). Also check the **Secured** box.
>
> For **host**, login to the portal, go to your Environment > Go to Application > Select the address of your T24 environment (example: *https://t24-1abbbbbccc900.temenos.cloud*)
