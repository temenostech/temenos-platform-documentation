...
| client_files\sqldb    | The SQL database will be   uploaded to the client_files\sqldb folder. The file extensions of the   database must be .bak, .zip or both (i.e. in case of more .bak files need to   uploaded, you can add them under a .zip which would also reduce the size   during Git deploy). Each file must contain the database name, the database   credentials, one or many DBs as well as the SQL server version.                                                                                                                                         |
<br>
